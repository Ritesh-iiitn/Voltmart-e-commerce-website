...
function ProductDetails({ addToCart }) {
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        const { data } = await axios.get(`https://voltmart-api.vercel.app/api/products/${id}`);
        if (data) {
          setProduct(data);
    ...
    async function fetchRecommended() {
      setRecLoading(true);
      try {
        const { data: recs } = await axios.get(`https://voltmart-api.vercel.app/api/products/${id}/similar`);
        setRecommended(recs || []);
      } catch (err) {
